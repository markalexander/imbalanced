...
from ..samplers import RandomResampler, RandomClassResampler
...
class TestRandomClassResampler:
    """Tests for the RandomClassResampler class."""

    def test_sub_sampling(self) -> None:
        # 1/2 of class 0 (0.5 * 6 = 3) + all of class 1 (4) = 7
        assert 7 == len(RandomClassResampler(dataset, 0, 0.5).indices)
        # 1/3 of class 0 (0.3... * 6 = 2) + all of class 1 (4) = 6
        assert 6 == len(RandomClassResampler(dataset, 0, 1/3).indices)
        # all of class 0 (6) + 40% of class 1 (0.4 * 4 = 2) = 8
        assert 8 == len(RandomClassResampler(dataset, 1, 0.4).indices)

    def test_super_sampling(self) -> None:
        # all of class 0 (6) + 150% of class 1 (1.5 * 4 = 6) = 12
        assert 12 == len(RandomClassResampler(dataset, 1, 1.5).indices)
        # all of class 0 (6) + 120% of class 1 (1.2 * 4 = 5) = 11
        assert 11 == len(RandomClassResampler(dataset, 1, 1.2).indices)

    def test_identity_sampling(self) -> None:
        assert 10 == len(RandomClassResampler(dataset, 0, 1.0).indices)
        assert 10 == len(RandomClassResampler(dataset, 1, 1.0).indices)

    def test_rejects_negative_rate(self) -> None:
        with pytest.raises(AssertionError):
            RandomClassResampler(dataset, 0, -0.1)
